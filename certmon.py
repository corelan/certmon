...
def checkcerts(certconfigfile, mailconfigfile, alertbefore, showverbose):
    serverlist = CertmonConf(certconfigfile).getServerlist()
    ...
    if len(serverlist) > 0:
        ...
        print("[+] Found %d entries in the cert config file at %s" %
              (len(serverlist), certconfigfile))

        for targetrecord in serverlist:
            targetips = getIPs(targetrecord)

            for thisip in targetips:
                print(
                    "\n[+] Checking %s (%s:%d)" %
                    (thisip, targethost, targetport))
                target = (thisip, targetport)
                try:
                    certinfo = ssl.get_server_certificate(target)
                    certinfo = certinfo.replace("=-----END", "=\n-----END")
                    x509 = OpenSSL.crypto.load_certificate(
                        OpenSSL.crypto.FILETYPE_PEM, certinfo)
                    expirdatestr = str(x509.get_notAfter())
                    expirdate = datetime.datetime.strptime(
                        expirdatestr.replace("b'", "").replace("'", ""), "%Y%m%d%H%M%SZ")
                    ...
                    if delta.days < 0:
                        print("    *** CERTIFICATE HAS EXPIRED ON %s (%d days ago) ***" %
                              (expirdate, (delta.days * -1)))
                        thismsg = createMsg(
                            x509, targethost, targetport, thisip, expirdate, delta.days)
                        expirlist.append(thismsg)
                    else:
                        print("    Cert expiration OK")
                        print("    Note: Certificate will expire on %s (%d days from now)" %
                              (expirdate, delta.days))
                        if delta.days < alertbefore:
                            print(
                                "    ** Warning: certificate will expire in less than %d days" % alertbefore)
                            thismsg = createMsg(
                                x509, targethost, targetport, thisip, expirdate, delta.days)
                    ...
                    if len(fieldcheck) > 0:
                        for fieldname in fieldcheck:
                            if fieldname == "issuer":
                                if not fieldcheck[fieldname] in str(
                                        issuer).lower():
                                    issuerok = False
                                    certchanged = True
        ...
        if len(expirlist) > 0:
            ...
            mailhandler.sendmail(
                content, mailsubject="Expired Certificates Alert (certmon.py)")
        ...
        if len(warnlist) > 0:
            ...
            mailhandler = Mailer(mailconfigfile)
            mailhandler.sendmail(
                content, mailsubject="Upcoming Certificate Expiration Warning (certmon.py)")
        ...
        if len(changedlist) > 0:
            ...
            mailhandler = Mailer(mailconfigfile)
            mailhandler.sendmail(
                content, mailsubject="Certificate Change Alert (certmon.py)")
